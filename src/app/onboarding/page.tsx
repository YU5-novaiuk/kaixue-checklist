...
export default function Onboarding(){
 ...
 const steps=useMemo(()=>(p.studentStatus==='new'?['status','stage','gender','accommodation','distance','training','date']:['status','stage','gender','accommodation','distance','date']),[p.studentStatus]);const current=steps[step]||steps[0];const canContinue=current!=='gender'||!!p.gender
 const finish=()=>{saveProfile({...p,educationStage:p.educationStage||'undergraduate',militaryTraining:p.studentStatus==='new'?(p.militaryTraining||'unknown'):'no',needsIdentityConfirmation:false});router.push('/')}
 const chooseStatus=(studentStatus:UserProfile['studentStatus'])=>setP({...p,studentStatus,educationStage:p.educationStage||'undergraduate',militaryTraining:studentStatus==='new'?(p.militaryTraining||'unknown'):'no'})
 ...
 {current==='stage'&&<Step title="你目前处于哪个阶段？" lead="不同阶段会获得对应的开学或返校准备事项。"><Choice value={p.educationStage||'undergraduate'} options={[["secondary","初高中"],["undergraduate","专本科"],["postgraduate","研究生"]]} onChange={v=>setP({...p,educationStage:v as UserProfile['educationStage']})}/></Step>}
 {current==='gender'&&<Step title="你的性别" lead="用于补充适合你的基础个人用品。"><Choice value={p.gender||''} options={[["female","女"],["male","男"]]} onChange={v=>setP({...p,gender:v as UserProfile['gender']})}/></Step>}
 ...
 </div><footer className="onboard-actions">{step>0?<button className="secondary" onClick={()=>setStep(step-1)}><ArrowLeft/>上一步</button>:<span/>}{step<steps.length-1?<button className="primary" disabled={!canContinue} onClick={()=>setStep(step+1)}>继续<ArrowRight/></button>:<button className="primary" disabled={!p.gender} onClick={finish}>生成我的清单<Check/></button>}</footer></main>
}
