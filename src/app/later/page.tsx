...
export default function Later(){const {data,addItem}=useChecklist();const toast=useToast();if(data.profile.accommodation!=='dorm')return <AppShell><header className="page-header"><div className="with-back"><Link href="/" className="icon-btn"><ArrowLeft/></Link><div><p className="eyebrow">走读准备</p><h1>无需宿舍购物建议</h1></div></div><p className="lead">你的清单已按走读场景生成，首页不会展示宿舍用品建议。</p></header></AppShell>;return <AppShell><header className="page-header"><div className="with-back"><Link href="/" className="icon-btn"><ArrowLeft/></Link><div><p className="eyebrow">不急着买</p><h1>建议到校后再决定</h1></div></div><p className="lead">确认空间、尺寸与规定之后再购买，减少浪费。</p></header><div className="suggestion-list">{suggestions.map(([name,reason])=>{const exists=data.items.some(x=>x.name===name);return <article key={name}><ShoppingBag/><div><h2>{name}</h2><p>{reason}</p></div><button disabled={exists} onClick={()=>{addItem({id:`later-${Date.now()}`,name,categoryId:'dorm',itemType:'physical',priority:'optional',preparationStatus:'in_progress',purchaseStatus:'buy_after_arrival',purchaseStatusOverride:'buy_after_arrival',quantity:1,tags:['到校后'],note:reason,isSystemItem:false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()});toast('已加入清单')}}>{exists?<span>已加入</span>:<><Plus/>加入</>}</button></article>})}</div></AppShell>}
