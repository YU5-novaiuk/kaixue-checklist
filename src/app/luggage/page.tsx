...
export default function LuggagePage(){
 ...
 const visible=getVisibleItems(data.items,data.categories);const packable=visible.filter(i=>i.itemType==='physical'&&i.luggageId!=='campus_parcel_station');const packedCount=packable.filter(isPackedForLuggage).length;const remaining=Math.max(0,packable.length-packedCount);const percent=packable.length?Math.round(packedCount/packable.length*100):0;const stationItems=visible.filter(i=>i.luggageId==='campus_parcel_station');const essential=visible.filter(i=>i.priority==='essential').slice(0,12);const unchecked=essential.filter(i=>!isDone(i)).length
 ...
 const selectedItems=selected?visible.filter(i=>i.luggageId===selected.id):[]
